...
public class Team {
	...
	public ModuleCollection<ObjectiveModule> getObjectives() {
		ModuleCollection<ObjectiveModule> objectives = new ModuleCollection<>();
		boolean wool = MatchHandler.getMatchHandler().getMatch().getModules().isModuleLoaded(WoolObjective.class);
		if (wool) {
			for (WoolObjective w : WoolObjective.getObjectives(this)) {
				objectives.add(w);
			}
		}
