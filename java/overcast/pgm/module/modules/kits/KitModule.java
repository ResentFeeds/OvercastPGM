...
public class KitModule extends Module {
	...
	public KitModule(String id, boolean force, boolean clear, boolean clearItems, List<String> parents,
			List<ItemKit> items, List<ArmorKit> armor, List<PotionKit> potions, List<BookKit> books,
			@Nullable HealthKit health, @Nullable HungerKit hunger, GamemodeKit gamemode, float walkspeed) {
		this.id = id;
		this.force = force;
		this.clear = clear;
	...
	public void applyKit(OvercastPlayer p) {
		...
		if (!this.books.isEmpty()) {
			for (BookKit books : getBooks()) {
				books.apply(p, this.force);
			}
